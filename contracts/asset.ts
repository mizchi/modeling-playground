import type { WindowSpec, AssetSpec } from './asset.types.ts';
export type { Vec3, BoneSpec, SocketSpec, ColliderSpec, AttackSpec, EmitterSpec, WindowSpec, ClipSpec, AssetSpec } from './asset.types.ts';
const fail=(path: string,message: string): never=>{throw new Error(`Asset contract ${path}: ${message}`);};
const fields=(value: unknown,keys: string[],path: string)=>{
  if(!value || typeof value!=='object' || Array.isArray(value))fail(path,'expected object');
  for(const key of keys)if(!Object.hasOwn(value as object,key))fail(path,`missing ${key}`);
  for(const key of Object.keys(value as object))if(!keys.includes(key))fail(path,`unknown ${key}`);
};
const list=<T>(value: T[],path: string): T[]=>{if(!Array.isArray(value)||value.length>512)fail(path,'expected array (max 512)');return value;};
const number=(value: number,path: string,min=-Infinity)=>{if(!Number.isFinite(value)||value<min)fail(path,'invalid number');};
const positive=(value: number,path: string)=>{number(value,path);if(value<=0)fail(path,'must be positive');};
const vector=(value: number[],path: string)=>{if(!Array.isArray(value)||value.length!==3)fail(path,'expected xyz');value.forEach(n=>number(n,path));};
const name=(value: unknown,path: string)=>{if(typeof value!=='string'||!/^[A-Za-z][A-Za-z0-9_-]*$/.test(value))fail(path,'invalid identifier');};
const unique=<T extends object,K extends keyof T>(items: T[],key: K,path: string)=>{
...
const reference=(value: string,ids: Set<string>,path: string)=>{if(!ids.has(value))fail(path,`missing reference ${value}`);};
...
export function validateAssetSpec(value: unknown): AssetSpec {
  ...
  const bones=unique(spec.rig.bones,'name','bones'),ordered=new Set<string>();
