import type { BaseTopology } from './base-topology.types.ts';
export type { BaseTopology } from './base-topology.types.ts';
/** Version 1: meters, Y-up, +Z forward; faces index positions/weights directly. */
export function validateBaseTopology(value: unknown): BaseTopology {
  const data=value as BaseTopology;
  ...
  const edges=new Map<string,{count:number;direction:number}>(),adj=data.positions.map(()=>new Set<number>());
