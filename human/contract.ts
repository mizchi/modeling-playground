import type { HumanModel, HumanShape, HumanBodyShape, HumanRig, HumanRecipe } from './contract.types.ts';
export type { HumanModel, HumanShape, HumanBodyShape, HumanRig, HumanRecipe } from './contract.types.ts';
...
export function presetRecipe(model: HumanModel): HumanRecipe {
  ...
  return {version:1,model,hair:model==='lumi'?'lumi-short':'none',face:model==='lumi'?'lumi':'clay',
    shape:Object.fromEntries(Object.keys(SHAPE_FIELDS).map(k=>[k,0])) as unknown as HumanShape,
    bodyShape:Object.fromEntries(Object.keys(BODY_SHAPE_FIELDS).map(k=>[k,0])) as unknown as HumanBodyShape,rig:null};
}
export function validateRig(input: unknown): HumanRig {
  const value=input as HumanRig;
  ...
  const bones=BASE45_BONES.map(reference=>{
    ...
    return {name:b.name,parent:b.parent,position:[...b.position] as [number,number,number]};
  });
  return {version:1,bones};
}
export function validateRecipe(input: unknown): HumanRecipe {
  const value=input as HumanRecipe;
  ...
  for(const [key,{min,max}] of Object.entries(SHAPE_FIELDS)) {
    const n=value.shape[key as keyof HumanShape];if(!Number.isFinite(n)||n<min||n>max)throw new Error(`${key} は ${min}〜${max} の数値が必要です`);
    result.shape[key as keyof HumanShape]=n;
  }
  ...
  if(Object.hasOwn(value,'bodyShape')) {
    ...
    for(const [key,{min,max}] of Object.entries(BODY_SHAPE_FIELDS)) {
      ...
      const n=value.bodyShape[key as keyof HumanBodyShape];if(!Number.isFinite(n)||n<min||n>max)throw new Error(`${key} は ${min}〜${max} の数値が必要です`);
      result.bodyShape[key as keyof HumanBodyShape]=n;
    }
  }
