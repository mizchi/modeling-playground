...
export function createHuman(input) {
  ...
  const root=recipe.hair==='lumi-short'||recipe.face==='lumi'?createLumi():createBase45();
  ...
  const edited=recipe.rig||Object.values(recipe.shape).some(v=>v!==0);
  if(edited) {
    ...
    const delta=new Map(BASE45_BONES.map(({name})=>{
      const shaped=shapePoint(rest.get(name),recipe.shape,'rig');
      return [name,rig.has(name)?rig.get(name).map((v,k)=>v-shaped[k]):[0,0,0]];
    }));
    for(const bone of bones) {
      const p=shapePoint(rest.get(bone.name),recipe.shape,'rig'),d=delta.get(bone.name)??delta.get('Head');
    ...
    root.traverse(mesh=>{
      ...
      for(let i=0;i<p.count;i++) {
        const point=shapePoint([p.getX(i),p.getY(i),p.getZ(i)],recipe.shape,mesh===body?'body':'hair');
        for(let j=0;j<4;j++) {
