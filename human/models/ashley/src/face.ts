import type { PixelPainter } from '../../../../modeling/types.ts';
...
function forwardEyeDepth(x: number,y: number,envelopeZ: number) {
...
export function facialPoint(x: number,y: number) {
...
const pixelY=(y: number)=>Math.round((FACE_ANATOMY.top-y)/(FACE_ANATOMY.top-FACE_ANATOMY.bottom)*95);

/** One continuous facial envelope; no eye-height-specific forward extrusion. */
export function facialFrontFactor(x: number) {
  return 1-Math.abs(x)**3;
}

/** X is normalized to the half-width of the corresponding facial ring. */
export function facialRelief(x: number,y: number) {
...
export function paintAshleyFace(p: PixelPainter) {
  ...
  for(const side of [0,1]) {
    const mirror=(x: number)=>side?95-x:x;
    const poly=(points: number[][],color: string)=>p.poly(points.map(([x,y])=>[mirror(x),y]),color);
