import type { Object3D, BufferGeometry, Mesh, BufferAttribute, InterleavedBufferAttribute } from 'three';
type Parts = ReturnType<typeof lowpolyParts>;
...
import { lowpolyParts, texturedGeometry, computeWeldedNormals } from '../../../../modeling/lowpoly.ts';
import { createAshleyAtlas, ASHLEY_TILES } from './texture.ts';
import { FACE_ANATOMY, FACE_SECTIONS, facialPoint } from './face.ts';
import { FACE_COLUMNS, createFaceTopology } from './topology.ts';
...
function group(parent: Object3D | null | undefined, name: string) {
...
export function createAshley() {
  ...
  for(const [label,s] of [['Left',-1],['Right',1]] satisfies [string, number][]) {
...
function makeWaistBelt(parent: Object3D,p: Parts) {
  const bottom=1.039,top=1.090,sides=8,targets=parent.children.filter(o=>isMesh(o));
  ...
  const planes=Array.from({length:sides},(_,i)=>{
    ...
    const support=(y: number)=>Math.max(...points.filter(p=>Math.abs(p.y-y)<1e-6).map(p=>x*p.x+z*p.z));
  ...
  const outlineAt=(y: number)=>planes.map((b,i)=>{
  ...
  for(const [y,inner] of [[bottom,true],[bottom,false],[top,false],[top,true]] satisfies [number,boolean][])for(let i=0;i<=sides;i++) {
...
function makeTrouserRise(parent: Object3D,p: Parts) {
...
function makeTrouserLeg(parent: Object3D,p: Parts,label: string,sign: number) {
...
function makeFace(parent: Object3D,p: Parts) {
  ...
  const positions: number[]=[],uvs: number[]=[],indices: number[]=[],n=rings[0].length;
  function triangle(points: number[][],coords: number[][],tile: string) {
    const offset=positions.length/3;
    points.forEach((point,i)=>{positions.push(...point);uvs.push(...p.uv(tile,coords[i][0],coords[i][1]));});indices.push(offset,offset+1,offset+2);
  }
...
function filterTriangles(geometry: BufferGeometry,keep: (point: Vector3)=>boolean) {
...
function joinNeckRoot(torso: Mesh,neck: Mesh) {
  const height=1.545;
  filterTriangles(torso.geometry,center=>center.y<height-1e-6);
  filterTriangles(neck.geometry,center=>center.y>height+1e-6);
  const vertices: {point:Vector3;normal:BufferAttribute | InterleavedBufferAttribute;index:number}[]=[];
  ...
  vertices.forEach((vertex,i)=>vertex.normal.setXYZ(vertex.index,sums[i].x,sums[i].y,sums[i].z));
}

function fittedBib(parent: Object3D,p: Parts,torso: Mesh) {
  fittedCloth(parent,p,torso,'IvoryBib',[[1.035,.094],[1.12,.102],[1.19,.109],[1.26,.126],[1.39,.153],
    [1.454,.131],[1.47,.104],[1.503,.041]],'bib',1);
}

function fittedCloth(parent: Object3D,p: Parts,torso: Mesh,name: string,outline: number[][],tile: string,direction: number) {
  ...
  const signed=(a: {x:number;y:number},b: {x:number;y:number},q: {x:number;y:number})=>(b.x-a.x)*(q.y-a.y)-(b.y-a.y)*(q.x-a.x);
  const clip=(polygon: Vector3[],a: Vector2,b: Vector2,orientation: number)=>{
    const result=[];
  ...
  const widthAt=(y: number)=>{
    const index=Math.max(0,Math.min(outline.length-2,outline.findIndex((row: number[])=>row[0]>=y)-1));
  ...
  for(let i=0;i<source.index!.count;i+=3) {
    ...
    if(direction*signed(triangle[0],triangle[1],triangle[2])<1e-10)continue;
    for(const mask of masks) {
      const orientation=Math.sign(signed(mask[0],mask[1],mask[2]));let polygon=triangle;
      ...
      for(let j=1;j<polygon.length-1;j++) {
        ...
        if(Math.abs(signed(tri[0],tri[1],tri[2]))<1e-8)continue;
...
function makeScalp(parent: Object3D,p: Parts,face: Object3D) {
  const hairline=[
...
function halterStrap(parent: Object3D,p: Parts,targets: Object3D[],name: string,sign: number) {
  ...
  const ray=new Raycaster(),project=(point: Vector3)=>{
...
function surfaceRibbon(parent: Object3D,p: Parts,targets: Object3D[],name: string,a: number[],b: number[],width: number,tile: string,clearance=.014) {
