...
import { FACE_SECTIONS, FACE_ANATOMY, EYE_SURFACE } from './face.ts';

export const FACE_COLUMNS=Object.freeze([-1,-.90,-.78,-.45,-.20,0,.20,.45,.78,.90,1]);

export interface FacialFeature { name: string; loops: number[][]; center?: number }
export interface FaceTopology { points: number[][]; quads: number[][]; triangles: number[][]; features: FacialFeature[] }
 ...
 */
export function createFaceTopology(): FaceTopology {
  const points:number[][]=[],quads:number[][]=[],triangles:number[][]=[],features:FacialFeature[]=[],columns=FACE_COLUMNS.length;
  const grid=FACE_SECTIONS.map(([y])=>FACE_COLUMNS.map(x=>{points.push([x,y]);return points.length-1;}));
  const column=(x: number)=>FACE_COLUMNS.indexOf(x);
  const patches=[
  ...
  const quad=(a: number,b: number,c: number,d: number)=>{quads.push([a,b,c,d]);triangles.push([a,b,c],[a,c,d]);};
  ...
  const indices=(ids:number[])=>ids.map(id=>remap.get(id)!);
