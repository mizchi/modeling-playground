...
export function createAsterHair() {
  ...
  for(const chain of chains) {
    ...
    chain.points.forEach((p,i)=>{
      ...
      b.position.fromArray(p.map((v,k)=>v-origin[k]));parent.add(b);bones.push(b);parent=b;origin=p;
    });
  }
  ...
  const isCrest=(i: number)=>i<7?i%2===1:crests.has(i-7);
  ...
  const rearPoints=[...outer,...innerBack],cols=backRoot.length,count=outer.length,links=new Map<string,Weight[]>();
  ...
  const rearWeights=(v:Point):Weight[]=>links.get(v.join(','))!;
  ...
  const key=(p:Point)=>p.map((v: number)=>v.toFixed(5)).join(','),tints=new Map<string,number[]>(),base=new Color(p.hair);
  points.forEach((v,i)=>{
