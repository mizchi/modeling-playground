...
export const ASTER_ATLAS=Object.freeze({version:1,size:256,active:'Neutral',
  tiles:['Neutral','Happy','Angry','Surprised','Blink','Wink'].map((name,i)=>({name,rect:[i%3*80,Math.floor(i/3)*80,80,80] as Tile}))});
export const asterFaceUV=(x: number,y: number): Vec2=>[(x+.27)/.54,(y-1.54)/.46];
...
export function createAsterAtlas() {
  ...
  for(const {name,rect:[ox,oy,w,h]} of ASTER_ATLAS.tiles) {
    ...
    const xy=([x,y]: number[]): Vec2=>{const [u,v]=asterFaceUV(x,y);return [ox+.5+u*(w-1),oy+.5+(1-v)*(h-1)];};
