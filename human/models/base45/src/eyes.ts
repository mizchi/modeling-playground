...
export const base45FaceDepth=(x: number,y: number)=>.184-1.6*x*x+.018*(y-1.932);
...
export function appendBase45Eyes(builder: TopologyBuilder, eyeRows: number[][]) {
  ...
  for(const side of [1,-1]) {
    ...
    const loop=(width: number,height: number,inset: number)=>circle.map(([u,v])=>{
