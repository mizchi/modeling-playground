import type { InspectionSurface } from './inspection.types.ts';
export type { InspectionSurface } from './inspection.types.ts';
...
export const faceCheckUV=([x,y]: readonly number[]): [number,number]=>[.5+x/SPAN,(TOP-y)/SPAN];

export function createFaceCheckTexture(surface: Exclude<InspectionSurface,'clay'>) {
...
export function createBase45Inspection({surface='clay'}: {surface?: InspectionSurface}={}) {
  if(!['clay','eyes','grid'].includes(surface))throw new Error('Invalid inspection surface');
  const data=createBase45Topology(),original=createBase45(),sourceGeometry=(original.getObjectByName('BaseBody') as Mesh).geometry;
  const selected=data.faces.map((face,i)=>({face,region:data.regions[i]}))
    .filter(({region})=>region.startsWith('Head')||region==='Neck');
  const positions: number[]=[],normals: number[]=[],uv: number[]=[],faces=[],sourceVertices: number[]=[],paintedFaces=[],mapping=new Map<string,number>();
