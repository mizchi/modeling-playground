...
export function appendPostauricularColumns(builder: TopologyBuilder,original: number[]) {
  const result: Record<'positive' | 'negative',number[]>={positive:[],negative:[]};
  for(const [side,front,back,key] of [[1,3,4,'positive'],[-1,9,8,'negative']] as const){
...
export function bridgePostauricular(builder: TopologyBuilder,previous: ReturnType<typeof appendPostauricularColumns> | undefined,next: ReturnType<typeof appendPostauricularColumns>,neckLoop: number[],earOpening: boolean) {
  for(const [key,name,index,opening] of [['positive','Left',3,0],['negative','Right',8,2]] as const){
