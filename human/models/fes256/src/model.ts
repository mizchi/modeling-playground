import type { CompactMeshBuilder, SurfaceColor, Influences } from '../../../../modeling/types.ts';
...
import { FES256, FES256_BONES } from './definition.ts';
import { createFes256Face } from './face.ts';
import { FES256_HEAD, headSurfaceDepth } from './head.ts';

/** Rings with shared corners and n-2 cap triangles; hidden caps count in the budget. */
function ringSolid(mesh:CompactMeshBuilder,rings:number[][][],color:SurfaceColor,bone:Influences,caps=[true,true]) {
...
const rectangle=(x: number,y: number,z: number,w: number,d: number)=>[[x-w,y,z-d],[x+w,y,z-d],[x+w,y,z+d],[x-w,y,z+d]];
...
export function createFes256() {
  ...
  const bones=new Map<string,Bone>(),anchors=new Map(FES256_BONES.map(b=>[b.name,b.position]));
  ...
  for(const [name,builder] of [['HeadSkin',head],['Hair',hair]] satisfies [string,CompactMeshBuilder][]) {
