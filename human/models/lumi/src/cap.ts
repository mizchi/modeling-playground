...
function boundary(faces: number[][]) {
...
const corners=(faces: number[][])=>{
  const counts=new Map<number,number>();for(const id of faces.flat())counts.set(id,(counts.get(id)??0)+1);
...
export function lumiCapFaces(data: BaseTopology) {
  ...
  const coarse=[],removed=new Set<number>();
  ...
  const apex=data.positions.findIndex((p: number[])=>Math.abs(p[1]-2.20)<1e-8);
  ...
  const shoulder=(id: number)=>data.positions[id][1]>2.13&&data.positions[id][1]<2.17;
  const partners=new Map<string,number>(),skip=new Set<number>(),result=[];
