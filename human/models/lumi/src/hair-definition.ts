export interface FringeGuide {width:number;rows:number[][];depth?:number;lift?:number}
...
export function lumiForeheadZ(x: number,y: number) {
...
export const LUMI_FRINGE: FringeGuide[]=tips.map((tip,i)=>({
  width:1,
