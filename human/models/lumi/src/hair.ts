...
import type { Point, Weight } from '../../../../modeling/types.ts';
import { Bone, Skeleton, SkinnedMesh, MeshStandardMaterial, Vector3 } from 'three';
import { compactMesh } from '../../../../modeling/compact-mesh.ts';
import { LUMI_FRINGE, LUMI_TENDRIL, lumiForeheadZ } from './hair-definition.ts';
import { createLumiHairTexture } from './hair-texture.ts';
import { lumiCapFaces } from './cap.ts';

const ORIGIN=1.76,local=(p: number[])=>[p[0],p[1]-ORIGIN,p[2]];
const GOLD='#ffffff';
...
export function createLumiHair(data: BaseTopology,sourceGeometry: BufferGeometry) {
  ...
  for(const c of chains) {
    ...
    c.points.map(local).forEach((p,i)=>{
      ...
      bone.position.fromArray(p.map((v,k)=>v-origin[k]));parent.add(bone);parent=bone;origin=p;bones.push(bone);
    });
  }
  ...
  const capPoints=data.positions.map((p: number[],i: number)=>{
  ...
  function lock(rows: number[][],theta: number,width: number,depth: number,name?: string,fitCrown=false,lift=.022,thin=false) {
    ...
    const points: number[][]=[],uvs: number[][]=[],links=new Map<string,Weight[]>(),tangent=[Math.cos(theta),0,-Math.sin(theta)],out=[Math.sin(theta),0,Math.cos(theta)];
    const lengths=[0];
    ...
    rows.forEach((p,row)=>{
      ...
      for(const [u,v] of row===rows.length-1?[[0,0]]:section) {
        ...
        if(fitCrown&&p[1]>2.0) {
          ...
          if(surface!==undefined&&Number.isFinite(surface))q[2]=surface+lift+v*thickness;
        }
    ...
    const weights=(p: Point)=>links.get(p.join(','))!;
