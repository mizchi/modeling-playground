...
import { createBase45, createBase45Topology } from '../../base45/src/model.ts';
import { createLumiTexture, lumiFaceUV } from './texture.ts';
import { createLumiHair } from './hair.ts';
...
export function createLumi() {
  const root=createBase45(),data=createBase45Topology(),body=root.getObjectByName('BaseBody') as SkinnedMesh<BufferGeometry,MeshStandardMaterial>,source=body.geometry;
  root.name='LUMI';root.userData={...root.userData,modelId:'lumi',base:'base45',baseCheckpoint:'29e4571'};
  const mapping=new Map<string,number>(),sourceVertices: number[]=[],faces=[],uv: number[]=[];
