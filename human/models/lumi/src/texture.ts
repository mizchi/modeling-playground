import type { Vec2 } from '../../../../modeling/types.ts';
import { LinearFilter } from 'three';
import { pixelPainter } from '../../../../modeling/pixel-atlas.ts';
export const LUMI_SKIN='#efd6b9';
export const LUMI_EYE_CENTER_X=.092;
export const lumiFaceUV=([x,y]: readonly number[]): Vec2=>[.5+x/.48,(2.23-y)/.48];

export function createLumiTexture() {
  const p=pixelPainter(256);p.rect(0,0,256,256,LUMI_SKIN);
  const xy=(v: number[]): Vec2=>lumiFaceUV(v).map(n=>n*256) as Vec2;
  const poly=(points: number[][],color: string)=>p.poly(points.map(xy),color);
  const line=(points: number[][],color: string,width=1)=>points.slice(1).forEach((b,i)=>p.line(...xy(points[i]),...xy(b),color,width));
  const oval=(x: number,y: number,rx: number,ry: number,color: string)=>poly(Array.from({length:24},(_,i)=>[x+rx*Math.cos(i*Math.PI/12),y+ry*Math.sin(i*Math.PI/12)]),color);
  const mix=(a: number[],b: number[],t: number)=>a.map((v,i)=>Math.round(v+(b[i]-v)*t));
  const hex=(rgb: number[])=>'#'+rgb.map((v: number)=>Math.max(0,Math.min(255,v)).toString(16).padStart(2,'0')).join('');
  function iris(x: number,y: number) {
