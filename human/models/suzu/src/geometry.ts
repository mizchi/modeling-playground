...
export function group(parent: T.Object3D | null,name: string) {
  const node=new T.Group();node.name=name;node.userData.focusTarget=true;parent?.add(node);return node;
}
export function mesh(parent: T.Object3D,name: string,geometry: T.BufferGeometry,material: T.Material) {
  const node=new T.Mesh(geometry,material);node.name=name;parent.add(node);return node;
}
export function ellipsoid(parent: T.Object3D,name: string,at: number[],size: number[],material: T.Material) {
  const node=mesh(parent,name,new T.SphereGeometry(1,32,24),material);
  node.position.fromArray(at);node.scale.fromArray(size);return node;
}
export function line(parent: T.Object3D,name: string,points: number[][],radius: number,material: T.Material) {
  const curve=new T.CatmullRomCurve3(points.map(p=>new T.Vector3(...p)));
  return mesh(parent,name,new T.TubeGeometry(curve,Math.max(12,points.length*6),radius,8,false),material);
}
export function segment(parent: T.Object3D,name: string,a: number[],b: number[],r1: number,r2: number,material: T.Material) {
...
export function loft(parent: T.Object3D,name: string,rings: number[][],material: T.Material,{segments=64,pleats=0,amplitude=0,frontPower=1}={}) {
  ...
  for(const [row,reverse] of [[0,true],[rings.length-1,false]] satisfies [number,boolean][]) {
...
export function lock(parent: T.Object3D,name: string,points: number[][],widths: number[],depth: number,material: T.Material) {
  const curve=new T.CatmullRomCurve3(points.map(p=>new T.Vector3(...p)));
...
export function patch(parent: T.Object3D,name: string,points: number[][],material: T.Material,zAt: (x:number,y:number)=>number,{subdivisions=0}={}) {
  ...
  let geometry: T.BufferGeometry=new T.ShapeGeometry(shape,24);
...
export function ovalPoints(x: number,y: number,rx: number,ry: number,steps=64) {
