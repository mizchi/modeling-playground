type SuzuMaterials = Record<'hair' | 'hairLight' | 'gold' | 'ribbon' | 'cream',T.Material>;
import * as T from 'three';
import { requireMesh } from '../../../../modeling/scene-objects.ts';
import { group, ellipsoid, line, segment, loft, lock, patch, ovalPoints } from './geometry.ts';
...
function bodiceZ(x: number,y: number) {
...
export function faceZ(x: number,y: number) {
...
export function createSuzu() {
  ...
  const material=(name: string,color: string,unlit=false)=>{const m=unlit?new T.MeshBasicMaterial({color,side:T.DoubleSide}):new T.MeshStandardMaterial({color,roughness:.82,metalness:0,side:T.DoubleSide});m.name=name;return m;};
  ...
  patch(head,'Small nose',[[-.013,1.337],[0,1.363],[.013,1.337],[0,1.328]],skin,
    (x: number,y: number)=>faceZ(x,y)+.0003+.014*Math.exp(-((x/.007)**2+((y-1.34)/.01)**2)),{subdivisions:3});
...
function makeEye(head: T.Object3D,side: number,{ink,white,material}: {ink:T.Material;white:T.Material;material:(name:string,color:string,unlit?:boolean)=>T.Material}) {
  ...
  const limits=(x: number)=>{const u=(x-cx)/rx,bulge=Math.max(0,1-u*u);return [cy-.025*bulge**.7+side*u*.005,cy+.037*bulge**.6+side*u*.005];};
  const top=[],bottom=[];
...
function makeHair(root: T.Object3D,{hair,hairLight,gold,ribbon,cream}: SuzuMaterials) {
  ...
  requireMesh(parent,'Bob silhouette').geometry=cap.toNonIndexed();
  cap.dispose();
