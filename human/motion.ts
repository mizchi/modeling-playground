...
export function fitMotion(root: Object3D,source: AnimationClip): AnimationClip {
  if(!source||!Array.isArray(source.tracks))throw new Error('アニメーションがありません');
...
export function createMotions(root: Object3D): AnimationClip[] {
  const track=(name: string,axis: number[],angles: number[])=>new QuaternionKeyframeTrack(`${name}.quaternion`,[0,.5,1,1.5,2],angles.flatMap((a: number)=>new Quaternion().setFromAxisAngle(new Vector3(...axis),a).toArray()));
...
export function createNeckCheckMotion() {
  ...
  return new AnimationClip('首・頭チェック',9,([['Neck',neck],['Head',head]] as [string,number[][]][]).map(([name,poses])=>
