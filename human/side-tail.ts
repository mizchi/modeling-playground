...
import type { CompactMeshBuilder, Influences, Point, Weight } from '../modeling/types.ts';
import { Bone, MeshStandardMaterial, Skeleton, SkinnedMesh, Vector3 } from 'three';
import { compactMesh } from '../modeling/compact-mesh.ts';
import { createLumiHairTexture } from './models/lumi/src/hair-texture.ts';
...
function influences([,y]: Point): Weight[] {
...
function closedLock(builder: CompactMeshBuilder,rows: number[][],sides: number,color: string,weights: Influences) {
  const points: number[][]=[],uv: number[][]=[],faces: number[][]=[],rings: number[][]=[];
  const lengths=[0];
...
export function attachSideTail(root: Object3D) {
  ...
  for(const [name,position] of joints){
    const bone=new Bone();bone.name=name;bone.position.fromArray(position.map((v,k)=>v-origin[k]));
