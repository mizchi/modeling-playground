...
import { SHAPE_FIELDS, BODY_SHAPE_FIELDS, presetRecipe, validateRecipe, validateRig } from './contract.mjs';
import { createHistory } from './state.mjs';
import { createHuman, disposeHuman, exportRig } from './model.mjs';
import { humanFraming } from './framing.mjs';
...
const sliderGroups=[{section:'shape',fields:SHAPE_FIELDS,container:'shape-fields'},
...
function setView(name=currentView) {
  currentView=name;const frame=framing[faceFocus?'face':'body'];controls.target.set(...frame.target);
...
function sync(recipe) {
  for(const {section,fields} of sliderGroups)for(const key of Object.keys(fields)){
    $(key).value=recipe[section][key];$(`${key}-value`).value=recipe[section][key].toFixed(2);
  }
...
for(const group of sliderGroups)for(const [key,field] of Object.entries(group.fields)) {
  ...
  input.addEventListener('input',()=>{const recipe=history.value;recipe[group.section][key]=Number(input.value);try{rebuild(recipe);}catch(e){error(e);}});
  input.addEventListener('change',()=>change(r=>{r[group.section][key]=Number(input.value);}));
