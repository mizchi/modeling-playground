...
import type { Point, Influences, SurfaceColor, Weight } from './types.ts';
...
export function compactMesh(boneNames: string[], transform: (point: Point) => Point = point => point) {
  const positions: number[] = [], colors: number[] = [], joints: number[] = [], weights: number[] = [], indices: number[] = [], cache = new Map<string, number>();
  const uvs: number[] = []; let hasUV = false;
  const boneIds = new Map(boneNames.map((name, i) => [name, i]));
  function vertex(point: Point, color: ColorRepresentation, influences: Influences, uv?: Point) {
    const links: Weight[] = typeof influences === 'function' ? influences(point) : [[influences, 1]];
    const packed = links.map(([name, weight]): [number | undefined, number] => [boneIds.get(name), Math.round(weight * 255)]);
  ...
  function surface(points: readonly Point[], faces: number[][], color: SurfaceColor, influences: Influences, textureCoordinates?: readonly Point[]) {
  ...
  function loft(rows: readonly Point[], sides: number, color: SurfaceColor, influences: Influences, phase = 0) {
    const points: number[][] = [], faces: number[][] = [];
    rows.forEach((row, j) => {
  ...
  function convex(points: readonly Point[], faces: number[][], color: SurfaceColor, influences: Influences) {
  ...
  function polygon(points: readonly Point[], normal: Point, color: SurfaceColor, influences: Influences) {
