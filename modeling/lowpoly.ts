...
import type { Tiles, Point } from './types.ts';
import { BufferGeometry, Float32BufferAttribute, Mesh, BoxGeometry, Vector3, ShapeUtils, Vector2 } from 'three';
import { atlasUV } from './pixel-atlas.ts';

/** A small UV-aware geometry vocabulary. No character names or I/O. */
export function texturedGeometry(positions: number[],uvs: number[],indices?: number[]): BufferGeometry {
...
export function computeWeldedNormals(geometry: BufferGeometry): BufferGeometry {
...
export function lowpolyParts(material: Material,tiles: Tiles,size=256) {
  const uv=(tile: string,u: number,v: number)=>atlasUV(tiles[tile],u,v,size);
  function add(parent: Object3D,name: string,geometry: BufferGeometry){const mesh=new Mesh(geometry,material);mesh.name=name;parent.add(mesh);return mesh;}
  function remap(geometry: BufferGeometry,tile: string) {
  ...
  function box(parent: Object3D,name: string,at: Point,dimensions: Point,tile: string) {
    const mesh=add(parent,name,remap(new BoxGeometry(...dimensions),tile));mesh.position.fromArray(at);return mesh;
  }
  /** Rings: [height, x radius, z radius, z center=0, x center=0]. */
  function loft(parent: Object3D,name: string,rings: readonly Point[],tile: string,{sides=8,capBottom=true,capTop=true}={}) {
  ...
  function panel(parent: Object3D,name: string,points: readonly Point[],tile: string,thickness=.009) {
  ...
  function bar(parent: Object3D,name: string,a: Point,b: Point,width: number,depth: number,tile: string) {
  ...
  function lock(parent: Object3D,name: string,points: readonly Point[],widths: number[],tile='hair',depth=.018) {
