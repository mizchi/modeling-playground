import type { Tile, Vec2, Point } from './types.ts';
...
export function atlasUV(tile: Tile,u: number,v: number,size: number): Vec2 {
  const [x,y,w,h]=tile;
...
export function pixelPainter(size: number) {
  ...
  function rect(x: number,y: number,w: number,h: number,color: string){for(let j=y;j<y+h;j++)for(let i=x;i<x+w;i++)dot(i,j,color);}
  function line(x0: number,y0: number,x1: number,y1: number,color: string,width=1) {
  ...
  function poly(points: readonly Point[],color: string) {
