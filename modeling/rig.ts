import { isMesh, isSkinnedMesh, isBone } from './scene-objects.ts';
import type { Mesh, Object3D } from 'three';
import type { AssetSpec } from '../contracts/asset.ts';
import type { Rig, BoneMap } from './types.ts';
...
export function createRig(spec: AssetSpec,name=spec.id): Rig {
  validateAssetSpec(spec);
  const root=new Group();root.name=name;
  const bones: BoneMap=Object.create(null);
...
export function skinRigidParts(root: Object3D,bones: BoneMap): Skeleton {
  root.updateMatrixWorld(true);
  const inverseRoot=root.matrixWorld.clone().invert();
  const ordered=Object.values(bones),skeleton=new Skeleton(ordered),parts: Mesh[]=[];
  root.traverse(o=>{if(isMesh(o)&&!isSkinnedMesh(o))parts.push(o);});
  for(const part of parts) {
    let owner=part.parent;while(owner&&!isBone(owner))owner=owner.parent;
    const index=ordered.indexOf(owner as Bone);
