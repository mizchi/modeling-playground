...
import { ASTER, ASTER_BONES } from './aster-definition.mjs';
import { createAsterHead } from './aster-head.mjs';
import { createAsterHair } from './aster-hair.mjs';

function rings(builder,rows,color,influences,caps=true) {
...
const rect=(x,y,z,w,d)=>[[x-w,y,z-d],[x+w,y,z-d],[x+w,y,z+d],[x-w,y,z+d]];
...
export function createAster() {
  ...
  const bones=new Map(),anchors=new Map(ASTER_BONES.map(b=>[b.name,b.position]));
  for(const {name,parent,position} of ASTER_BONES) {
    const bone=new Bone();bone.name=name;const origin=anchors.get(parent)??[0,0,0];
    bone.position.set(...position.map((v,i)=>v-origin[i]));if(name==='Head')bone.userData.focusTarget=true;
    (bones.get(parent)??root).add(bone);bones.set(name,bone);
  }
  const p=ASTER.palette,body=compactMesh([...bones.keys()]);
  const torsoWeights=([,y])=>y<1.05?[['Hips',1]]:y>1.3?[['Chest',1]]:[['Hips',.5],['Chest',.5]];
  ...
  for(const side of [-1,1]) {
    ...
    const armWeights=([x,y])=>y>1.22?[[n+'UpperArm',1]]:y<1.10?[[n+'Forearm',1]]:[[n+'UpperArm',.5],[n+'Forearm',.5]];
    ...
    const legWeights=([,y])=>y>.60?[[n+'Thigh',1]]:y<.48?[[n+'Shin',1]]:[[n+'Thigh',.5],[n+'Shin',.5]];
