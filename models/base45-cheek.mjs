...
export function relaxBase45Cheek(data) {
  const neighbors=data.positions.map(()=>new Set()),fixed=new Set();
  ...
  const key=([x,y,z])=>[Math.abs(x),y,z].map(v=>v.toFixed(6)).join();
  ...
  for(let pass=0;pass<2;pass++){
    ...
    data.positions=before.map((p,i)=>{
      ...
      return p.map((v,k)=>v+(ids.reduce((sum,j)=>sum+before[j][k],0)/ids.length-v)*w*(k===1?1:corner));
    });
