...
import { BASE45, BASE45_BONES } from './base45-definition.mjs';
...
export function createBase45Topology() {
  ...
  const ring=(points,weights)=>points.map(p=>vertex(p,typeof weights==='function'?weights(p):weights));
  const torsoWeights=([,y])=>y<1.13?[['Hips',1]]:y<1.31?mix('Hips','Spine',(y-1.13)/.18):mix('Spine','Chest',(y-1.31)/.10);
  ...
  for(const s of [1,-1]) {
    ...
    for(const [y,x,w,front,back,z] of rows) {
      let weights=y>.625?mix('Hips',n+'Thigh',(1.015-y)/.1):y>.545?mix(n+'Thigh',n+'Shin',(.625-y)/.08):y>.16?[[n+'Shin',1]]:y>.09?mix(n+'Shin',n+'Foot',(.16-y)/.07):[[n+'Foot',1]];
      ...
      }),p=>p[1]<=.09&&p[2]>.13?mix(n+'Foot',n+'Toe',.5):weights);
    ...
    for(const [x,h,d] of [[.315,.078,.095],[.56,.061,.062],[.60,.06,.06],[.64,.063,.063],[.76,.06,.056]]) {
      const weights=x<.4?mix(n+'Clavicle',n+'UpperArm',(x-.265)/.135):x<.56?[[n+'UpperArm',1]]:x<.64?mix(n+'UpperArm',n+'Forearm',(x-.56)/.08):[[n+'Forearm',1]];
...
export function createBase45() {
  ...
  const bones=new Map(),anchors=new Map(BASE45_BONES.map(b=>[b.name,b.position]));
  for(const {name,parent,position} of BASE45_BONES) {
    const bone=new Bone();bone.name=name;bone.position.set(...position.map((v,i)=>v-(anchors.get(parent)?.[i]??0)));
