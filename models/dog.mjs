...
function fitFace(points, geometry, offset) {
  ...
  for(let i=0;i<indices.count;i+=3) {
    ...
    const boundary=[a,b,c].map(flatten),winding=Math.sign(edge(...boundary));
    let clipped=source;
...
export function createDog(preset = DOG) {
  ...
  const mark = (points,normal,color,kind) => {
