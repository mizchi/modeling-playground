...
import { EXPRESSION_NAMES } from './fes256-definition.mjs';

export const FES256_ATLAS=Object.freeze({version:1,size:256,active:'Neutral',tiles:['Neutral',...EXPRESSION_NAMES].map((name,i)=>({name,rect:[i%3*80,Math.floor(i/3)*80,80,80]}))});
export const faceUV=(x,y)=>[(x+.26)/.52,(y-1.10)/.40];
...
export function createFes256Atlas() {
  ...
  for(const {name,rect:[ox,oy,w,h]} of FES256_ATLAS.tiles) {
    const toPixel=([x,y])=>{const [u,v]=faceUV(x,y);return [ox+.5+u*(w-1),oy+.5+(1-v)*(h-1)];};
    const poly=(points,color)=>painter.poly(points.map(toPixel),color);
    const line=(points,color,width=1)=>points.slice(1).forEach((p,i)=>painter.line(...toPixel(points[i]),...toPixel(p),color,width));
    const ellipse=(x,y,rx,ry,color)=>poly(Array.from({length:24},(_,i)=>{const a=i/24*Math.PI*2;return [x+rx*Math.cos(a),y+ry*Math.sin(a)];}),color);
    ...
    for(const side of [-1,1]) {
      ...
      const arc=(width,height,tilt=0)=>Array.from({length:17},(_,i)=>{const t=i/16,s=-Math.cos(t*Math.PI);return [x+s*width,y+Math.sin(t*Math.PI)*height+tilt*s];});
