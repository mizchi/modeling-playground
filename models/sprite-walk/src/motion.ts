export interface WalkLimb { hip:number[];ankle:number[];knee:number[];shoulder:number[];wrist:number[];elbow:number[];contact:boolean }
export interface WalkPose {phase:number;pelvis:number[];chest:number[];head:number[];left:WalkLimb;right:WalkLimb}
import {getRig} from './definition.ts';

const add=(a: number[],b: number[])=>a.map((v,i)=>v+b[i]);
const sub=(a: number[],b: number[])=>a.map((v,i)=>v-b[i]);
const mul=(a: number[],s: number)=>a.map((v: number)=>v*s);
...
function joint(start: number[],end: number[],a: number,b: number,bend: number[]) {
...
function foot(phase: number,rig: ReturnType<typeof getRig>) {
...
export function sampleWalk(phase: number,proportion='legacy') {
  ...
  const pose={phase:p,pelvis,chest,head:[sway*.3,rig.headCenter+bounce,.038*s]} as WalkPose;
  for(const [side,sign,offset] of [['left',-1,0],['right',1,.5]] satisfies ['left' | 'right', number, number][]) {
