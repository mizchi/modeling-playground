import type { WalkPose } from './motion.ts';
export interface IndexedFrame {width:number;height:number;pixels:Uint8Array}
import {SPEC,PALETTE,DIRECTIONS,getRig} from './definition.ts';
import {sampleWalk} from './motion.ts';
import {pixelHead} from './pixel-head.ts';

const sub=(a: number[],b: number[])=>a.map((v,i)=>v-b[i]);
const cross=(a: number[],b: number[])=>[a[1]*b[2]-a[2]*b[1],a[2]*b[0]-a[0]*b[2],a[0]*b[1]-a[1]*b[0]];
const norm=(a: number[])=>{const d=Math.hypot(...a);return a.map((v: number)=>v/(d||1));};

/** Orthographic 3D projection with a small downward camera pitch. */
export function project(point: number[],direction: string) {
...
function ellipsoid(center: number[],radii: number[],base: number,transform?: (point:number[])=>number[]) {
...
function limb(a: number[],b: number[],radius: number,base: number) {
...
function geometry(pose: WalkPose,colored: boolean,rig: ReturnType<typeof getRig>) {
  ...
  for(const [side,index] of [['left',5],['right',8]] satisfies ['left' | 'right', number][]) {
...
export function renderFrame(phase: number,direction='w',{colored=true,proportion='legacy'}={}) {
...
export function toRgba(frame: IndexedFrame) {
