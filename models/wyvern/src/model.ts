...
import type { WyvernMaterials } from './geometry.ts';
import { Group, MeshStandardMaterial, MeshBasicMaterial, DoubleSide, Vector3 } from 'three';
import { WYVERN } from './definition.ts';
import { sweep, triangles, part, plate } from './geometry.ts';
export { WYVERN } from './definition.ts';
...
function makeHead(root: Object3D,materials: WyvernMaterials,p: typeof WYVERN.palette) {
...
function makeWing(root: Object3D,side: number,materials: WyvernMaterials,p: typeof WYVERN.palette) {
  const wing=part(root,side===1?'LeftWing':'RightWing',materials),mirror=(point: number[])=>[point[0]*side,point[1],point[2]];
  ...
  const addSurface=(geometry: BufferGeometry,...args: Parameters<typeof wing.add> extends [unknown,...infer Rest] ? Rest : never)=>{geometry.userData.wingSurface=true;wing.add(geometry,...args);};
...
function makeLeg(root: Object3D,side: number,materials: WyvernMaterials,p: typeof WYVERN.palette) {
  const leg=part(root,side===1?'LeftLeg':'RightLeg',materials),m=(point:number[])=>[point[0]*side,point[1],point[2]];
