...
import type { BoneRow, Vec3, Weight } from '../../../modeling/types.ts';
...
import { createWyvern, WYVERN } from './model.ts';
...
export const WYVERN_BONES = Object.freeze(([
  ...
  ...[1, -1].flatMap<BoneRow>(side => {
    ...
    const mirror = (p: number[]): Vec3 => [p[0] * side, p[1], p[2]];
    return [
      ...
      ...WYVERN.fingers.slice(0, 4).map<BoneRow>((tip, index) => {
...
function membraneInfluences(side: string, x: number, y: number, z: number, structural: Weight[]): Weight[] {
  ...
  const ribWeights = [[panelIndex, share], [panelIndex + 1, 1 - share]]
    .filter(([index]) => index < 4).map(([index, weight]): Weight => [`${side}Rib${index}`, flex * weight]);
  ...
  return [...structural.map(([name, weight]): Weight => [name, weight * anchor]),
    [`${side}Fan`, 1 - total - anchor] as Weight, ...ribWeights].filter(([, weight]) => weight > 0);
}

/** Shared vertex positions receive identical weights, including the duplicated flat-shaded faces. */
function influences(part: string, x: number, y: number, z: number): Weight[] {
...
export function createWyvernRig() {
  ...
  const bones = new Map<string,Bone>(), anchors = new Map(WYVERN_BONES.map(b => [b.name, b.position]));
  for (const definition of WYVERN_BONES) {
    ...
    bone.position.fromArray(definition.position.map((v, i) => v - parentAnchor[i]));
  ...
  for (const child of [...root.children]) if ('isGroup' in child && child.children.length === 0) child.removeFromParent();
  return root;
}
