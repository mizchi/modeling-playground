...
const options=(entries: string[][])=>entries.map(([id,label])=>({id,label}));
...
export const BASTION_SLOTS: {id: BastionSlot;label:string;position:number[];options:{id:string;label:string}[]}[]=freeze([
...
export type BastionSlot = 'core' | 'head' | 'leftArm' | 'rightArm' | 'leftLeg' | 'rightLeg' | 'backpack' | 'leftWeapon' | 'rightWeapon' | 'leftShoulder' | 'rightShoulder';
export type BastionLoadout = Record<BastionSlot,string>;
export const DEFAULT_LOADOUT: BastionLoadout=freeze({core:'fortress',head:'sensor',leftArm:'armored',rightArm:'armored',
...
export function validateLoadout(value: unknown): BastionLoadout {
  const loadout=value as BastionLoadout;
