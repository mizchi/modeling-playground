...
export function createKit(root: T.Object3D) {
  let serial=0;
  const cache=new Map<string,T.BufferGeometry>(),map=paintMap();
  const material=(color: string,metalness=.45,roughness=.65,extra={})=>new T.MeshStandardMaterial({color,metalness,roughness,...extra});
  const m={
  ...
  function geometry(key: string,build: ()=>T.BufferGeometry){if(!cache.has(key))cache.set(key,build());return cache.get(key)!;}
  function mesh(name: string,g: T.BufferGeometry,mat: T.Material,at=[0,0,0],rotation=[0,0,0],parent=root) {
    const node=new T.Mesh(g,mat);node.name=`${root.name}_${name}_${serial++}`;
    node.position.fromArray(at);node.rotation.set(rotation[0],rotation[1],rotation[2]);node.castShadow=node.receiveShadow=true;parent.add(node);return node;
  }
  function box(name: string,at: number[],size: number[],mat=m.armor,rotation=[0,0,0],parent=root) {
  ...
  function rod(name: string,a: number[],b: number[],r: number,mat=m.steel,r2=r,sides=12,parent=root) {
  ...
  function hull(name: string,rings: number[][],mat=m.armor,parent=root) {
  ...
  function bolt(at: number[],parent=root) {return rod('bolt',[at[0],at[1],at[2]-.018],[at[0],at[1],at[2]+.018],.025,m.steel,.025,6,parent);}
  function vent(name: string,at: number[],w: number,h: number,parent=root) {
  ...
  function barrel(name: string,at: number[],r: number,length: number,parent=root) {
    const [x,y,z]=at;
  ...
  function stencil(text: string,at: number[],width: number,rotation=[0,0,0],parent=root) {
  ...
  function hose(name: string,points: number[][],r=.04,parent=root) {
