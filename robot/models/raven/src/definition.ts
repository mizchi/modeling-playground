import type { EmitterSpec } from '../../../../contracts/asset.ts';
import { validateAssetSpec } from '../../../../contracts/asset.ts';

const bones: import('../../../../contracts/asset.ts').BoneSpec[]=[];
const bone=(name: string,parent: string | null,position: import('../../../../contracts/asset.ts').Vec3)=>bones.push({name,parent,position});
...
const collider=(id: string,bone: string,center: number[],halfExtents: number[])=>({id,bone,shape:'box',center,halfExtents});
...
const windows=(start: number,end: number)=>emitters.map(e=>({kind:'emitter',id:e.id,start,end}));
...
/** @type {import('../../../../contracts/asset.ts').AssetSpec} */
