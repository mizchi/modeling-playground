...
import { ravenClips } from './motion.ts';
import { RAVEN_SPEC } from './definition.ts';
...
export function createRaven() {
  ...
  const mat=(name: string,color: string,metalness=.3,emissive: string | null=null)=>{
  ...
  const joint=(parent: string,name: string,at: number[],radius: number,axis='x')=>{
    const mesh=add(parent,name,new T.CylinderGeometry(radius,radius,.14,12),dark,at);
    if(axis==='x')mesh.rotation.z=Math.PI/2;
    if(axis==='z')mesh.rotation.x=Math.PI/2;
    return mesh;
  };
  const fin=(parent: string,name: string,points: number[][],material: T.Material)=>plate(parent,name,points,.045,-.09,material);
  ...
  for(const [label,s] of [['Left',-1],['Right',1]] satisfies [string, number][]) {
    ...
    for(const [jet,length,radius] of [[label+'BackJet',.59,.113],[label+'FootJet',.30,.060]] satisfies [string,number,number][]) {
      for(const [suffix,factor,m] of [['Plume',1,jetMat],['HotCore',.72,jetCore]] satisfies [string,number,T.Material][]) {
