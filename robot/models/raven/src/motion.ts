...
import { solveHorizontalSweep } from '../../../../runtime/solvers.ts';
...
const SLASH_KEYS:{t:number;sweep:number;p:number[];thrust:number;r:Record<string,number[]>}[]=[
...
function horizontalSlashArm(rotations:Record<string,number[]>,sweep:number,weight:number) {
  const quaternion=(angles: number[])=>new Quaternion().setFromEuler(new Euler(...angles));
  ...
  const upper=new Euler().setFromQuaternion(result.upper,'XYZ');
  const forearm=new Euler().setFromQuaternion(result.forearm,'XYZ');
  rotations.RightUpperArm=[upper.x,upper.y,upper.z];
  rotations.RightForearm=[forearm.x,forearm.y,forearm.z];
}

/** Pose contract: XYZ Euler rotations in radians and root/jet transforms in meters. */
export function ravenPose(clip: string,time: number) {
