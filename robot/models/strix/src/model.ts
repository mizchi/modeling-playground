...
import { createRig, skinRigidParts } from '../../../../modeling/rig.ts';
import { rigidPrimitives } from '../../../../modeling/primitives.ts';
import { STRIX_GAIT as G, STRIX_LEGS, STRIX_SPEC, STRIX_RIGHT_ELBOW, STRIX_JETS, STRIX_IK } from './definition.ts';
import { limbFrame, strixClips } from './motion.ts';
...
export function createStrix() {
  ...
  const mat=(name: string,color: string,metalness=.55,roughness=.48)=>Object.assign(new T.MeshStandardMaterial({color,metalness,roughness}),{name});
  ...
  const rod=(parent:string,name:string,a:number[],b:number[],r:number,m:T.Material=steel,r2=r,sides=12)=>{
  ...
  const vent=(parent:string,name:string,at:number[],w:number,h:number)=>{
  ...
  for(const jet of STRIX_JETS) {
    ...
    for(const [suffix,factor,material] of [['Plume',1,flame],['Core',.66,hot]] satisfies [string,number,T.Material][]) {
  ...
  for(const leg of STRIX_LEGS) {
    ...
    const limb=(parent:string,start:number[],end:number[],build:()=>void,facing?:T.Vector3)=>{
  ...
  for(const part of bones.RightForearm.children)if(isMesh(part)) {
