...
import { STRIX_GAIT as G, STRIX_LEGS, STRIX_SPEC, STRIX_STANCE, STRIX_JETS, STRIX_BOOST } from './definition.ts';
import { strixBoost } from './boost.ts';
import { solveTwoBone } from '../../../../runtime/solvers.ts';
import { bakePoseClips } from '../../../../modeling/bake-motion.ts';
import type { Vec3 } from '../../../../modeling/types.ts';
export interface StrixFoot {hip: Vector3; knee: Vector3; ankle: Vector3; contact: boolean; phase: number; clamped: boolean}
export interface StrixPose {position: Vec3; rotations: Record<string, Vec3>; scales: Record<string, Vec3>; feet: Record<string, StrixFoot>}

const modulo=(v: number)=>((v%1)+1)%1;
...
export function limbFrame(direction: Vector3,facing=new Vector3(0,1,0)) {
...
const euler=(q: Quaternion): Vec3=>new Euler().setFromQuaternion(q).toArray().slice(0,3) as Vec3;
...
export function strixPose(name: string,time: number): StrixPose {
  ...
  const position=boost?.position??[0,bob,travel],origin=new Vector3(...position),feet: Record<string, StrixFoot>={};
  const rotations: Record<string, Vec3>=Object.fromEntries(STRIX_SPEC.rig.bones.filter(b=>b.name!=='Motion').map(b=>[b.name,[0,0,0]]));
  ...
  const scales: Record<string,Vec3>=Object.fromEntries(STRIX_JETS.map(jet=>{
    ...
    return [jet.name,[width,jet.name.includes('Lift')?length:width,jet.name.includes('Main')?length:width] as Vec3];
  }));
  return {position: position as Vec3,rotations,scales,feet};
}
