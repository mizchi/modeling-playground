...
import type { AssetSpec, WindowSpec } from '../contracts/asset.ts';
import { Matrix4, Quaternion, Vector3 } from 'three';
import { validateAssetSpec } from '../contracts/asset.ts';
import { activeWindows } from './timeline.ts';

/** Bind immutable authoring data to a loaded rig. No DOM, rendering or game rules. */
export function bindAsset(root: Object3D,clips: AnimationClip[],definition: AssetSpec) {
  ...
  const bones=new Map(spec.rig.bones.map(b=>{
    ...
    if(!isBone(node)||(isBone(node.parent)?node.parent.name:null)!==b.parent)throw new Error(`Asset bone hierarchy: ${b.name}`);
  ...
  const socketPosition=(id: string)=>sockets.get(id)!.getWorldPosition(new Vector3());
  return {
    ...
    sample(clipName: string,time: number) {
      const clip=spec.clips.find(c=>c.name===clipName);
      if(!clip)throw new Error(`Unknown asset clip: ${clipName}`);
      const windows=activeWindows(clip,time),active=(kind: WindowSpec['kind'],id: string)=>windows.some(w=>w.kind===kind&&w.id===id);
      root.updateMatrixWorld(true);
      return {
