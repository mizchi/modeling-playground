...
import type { Tile } from '../modeling/types.ts';
interface ExpressionSpec { version: 1; size: number; tiles: {name: string; rect: Tile}[]; active?: string }
...
export class ExpressionAtlas {
  ...
  constructor(mesh: Mesh) {
    const spec=mesh?.userData?.expressionAtlas as ExpressionSpec;
    if(!spec||spec.version!==1||!Number.isInteger(spec.size)||spec.size<1||spec.size>2048||
      !Array.isArray(spec.tiles)||!spec.tiles.length||Array.isArray(mesh.material)||!('map' in mesh.material)||!mesh.material.map||!mesh.geometry?.attributes.uv)
