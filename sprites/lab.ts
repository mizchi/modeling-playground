import { elementLookup } from '../viewer/dom.ts';
import type { PageElements } from './dom.types.ts';
import {SPEC,DIRECTIONS,PALETTE,PROPORTIONS,getRig,artifactNames,sampleWalk,renderFrame,project,toRgba} from '../models/sprite-walk/src/walk.ts';
const assets=import.meta.glob<string>('../models/sprite-walk/output/sprite-walk*.{png,json}',{query:'?url',import:'default',eager:true});
...
const tileCtx=tile.getContext('2d')!,cache=new Map<string,ImageData>();
const comparisons=(['8head','4head','3head','2head'] as const).map(id=>({id,canvas:$(`compare-${id}`),ctx:$(`compare-${id}`).getContext('2d')!}));
...
function selectProportion(id: string) {
  getRig(id);state.proportion=id;$('proportion').value=id;
  const paths=artifactNames(id);
  for(const key of ['debug','neutral','metadata'] as const) $(`${key}-download`).href=assets['../models/sprite-walk/output/'+paths[key]];
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-proportion]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.proportion===id)));
  draw();
}

function frameImage(frame: number,proportion=state.proportion) {
...
const color=(index: number)=>`rgb(${PALETTE[index].slice(0,3).join(',')})`;

function line(a: number[],b: number[],index: number) {
...
function draw() {
  ...
  for(const [side,index] of [['left',7],['right',10]] satisfies ['left' | 'right', number][]) {
    ...
    if(state.bones) for(const [a,b] of [['hip','knee'],['knee','ankle'],['shoulder','elbow'],['elbow','wrist']] as const) {
  ...
  $('frame').value=String(state.frame);$('frame-label').textContent=`${state.frame+1} / ${SPEC.frames}`;
...
$('frame').oninput=e=>{pause();state.frame=Number((e.target as HTMLInputElement).value);state.ticks=state.frame;draw();};
$('direction').onchange=e=>{state.direction=(e.target as HTMLInputElement).value;draw();};
$('proportion').onchange=e=>selectProportion((e.target as HTMLInputElement).value);
document.querySelectorAll<HTMLButtonElement>('[data-proportion]').forEach(button=>button.onclick=()=>selectProportion(button.dataset.proportion!));
$('speed').onchange=e=>{state.speed=Number((e.target as HTMLInputElement).value);};
for(const name of ['colored','bones','ground'] as const) $(name).onchange=e=>{state[name]=(e.target as HTMLInputElement).checked;draw();};
strip.onclick=e=>{
