...
import { RAVEN_SPEC } from '../robot/models/raven/src/definition.ts';
import { createRaven } from '../robot/models/raven/src/model.ts';
import { bindAsset } from '../runtime/asset.ts';
import { activeWindows, crossedEvents } from '../runtime/timeline.ts';
...
test('GLB and sidecar are generated together and bind after reimport',async()=>{
  const spec=JSON.parse(await readFile(new URL('../robot/models/raven/output/raven.asset.json',import.meta.url),'utf8'));
  assert.deepEqual(spec,RAVEN_SPEC);
  const bytes=await readFile(new URL('../robot/models/raven/output/raven.glb',import.meta.url));
