...
import { ASTER, ASTER_BONES } from '../models/aster-definition.mjs';
import { ASTER_FACE, createAsterHead } from '../models/aster-head.mjs';
...
test('head and hair export as independent, matching-origin GLBs',async()=>{
  ...
  for(const [name,part] of [['aster-head',head],['aster-hair',createAsterHair()]]) {
    const bytes=await readFile(new URL(`../output/parts/${name}.glb`,import.meta.url));
