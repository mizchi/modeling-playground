...
test('shared-index topology is one closed oriented surface, mostly quads',()=>{
  ...
  assert.ok(data.positions.length<=628,'Keep the editable base compact, including local eye and contour loops');
  assert.ok(data.faces.length<=626,'Spend loops on silhouette, joints and eyes, not uniform subdivision');
