...
import { BASE45 } from '../human/models/base45/src/definition.ts';
...
test('shared-index topology is one closed oriented surface, mostly quads',()=>{
  ...
  assert.ok(data.positions.length<=722,'Keep the base compact, including local ear attachments and one neck support row');
  assert.ok(data.faces.length<=720,'Spend loops on silhouette, joints, eyes and ears, not uniform subdivision');
...
test('quad OBJ and valid GLB retain the same reusable base and topology',async()=>{
  ...
  assert.deepEqual(Buffer.from(bytes),await readFile(new URL('../human/models/base45/output/base45.glb',import.meta.url)));
  assert.equal(obj,await readFile(new URL('../human/models/base45/output/base45.obj',import.meta.url),'utf8'));
  const delivered=JSON.parse(await readFile(new URL('../human/models/base45/output/base45.topology.json',import.meta.url),'utf8'));
