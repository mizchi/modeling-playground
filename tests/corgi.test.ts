...
import { createDog } from '../models/dog/src/model.ts';
import { DOG, CORGI, DOG_PRESETS, dogPoint } from '../models/dog/src/definition.ts';
...
for(const id of ['corgi','corgi-chibi'])test(`${id} exports using the shared generator and idle animation`,async()=>{
  ...
  const bytes=await readFile(assetUrl(`${id}.glb`));
