...
import { FACE_ANATOMY } from '../../human/models/ashley/src/face.ts';
...
test('Ashley: full-body view presets',async({page})=>{
  for(const [view,name] of [['正面','front'],['側面','side'],['背面','back'],['斜め','viewer']]) {
    ...
    await page.locator('canvas').screenshot({path:outputPath(`ashley-${name}.png`)});
  }
});
...
test('Ashley: back harness focus and rear quarter',async({page})=>{
  ...
  await backCanvas.screenshot({path:outputPath('ashley-harness-back.png')});
  await page.getByRole('button',{name:'背面',exact:true}).click();
  ...
  await page.mouse.up();
  await page.locator('canvas').screenshot({path:outputPath('ashley-rear-quarter.png')});
});
...
test('Ashley: face focus, wireframe and jaw quarter',async({page})=>{
  ...
  await canvas.screenshot({path:outputPath('ashley-face.png')});
  await page.getByLabel('ワイヤーフレーム',{exact:true}).check();
  await canvas.screenshot({path:outputPath('ashley-face-wireframe.png')});
  ...
  await page.mouse.up();
  await canvas.screenshot({path:outputPath('ashley-jaw-quarter.png')});
});
...
test('Ashley: head profile and neck transition',async({page})=>{
  ...
  await canvas.screenshot({path:outputPath('ashley-profile.png')});
  ...
  await canvas.screenshot({path:outputPath('ashley-neck-profile.png')});
});

test('Ashley: standalone GLB and embedded texture',async({page})=>{
  await page.getByLabel('GLBファイルを選択').setInputFiles(fileURLToPath(new URL('../../human/models/ashley/output/ashley.glb',import.meta.url)));
  await expect(page.locator('#status')).toHaveText('表示中');
  ...
  const decoded=await page.evaluate(async(upperLid)=>{
    ...
    const loaded=await new GLTFLoader().loadAsync('/human/models/ashley/output/ashley.glb');
