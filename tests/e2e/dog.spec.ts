...
for(const model of ['dog','corgi','corgi-chibi'])test(`${model} reads from all sides and plays the shared lightweight idle`,async({page})=>{
  ...
  for(const [label,file] of [['斜め','quarter'],['正面','front'],['側面','side'],['背面','back']]) {
    ...
    await page.locator('#viewport canvas').screenshot({path:outputPath(`${model}-${file}.png`)});
  }
  ...
  expect((await page.locator('#viewport canvas').screenshot({path:outputPath(`${model}-idle.png`)})).equals(initial)).toBe(false);
  await page.getByLabel('骨格を表示',{exact:true}).check();
  await page.locator('#viewport canvas').screenshot({path:outputPath(`${model}-rig.png`)});
  await page.getByLabel('骨格を表示',{exact:true}).uncheck();
  await page.setViewportSize({width:390,height:844});
  await page.getByRole('button',{name:'斜め',exact:true}).click();
  await page.locator('#viewport canvas').screenshot({path:outputPath(`${model}-mobile.png`)});
