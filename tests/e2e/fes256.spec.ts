...
test('FES-inspired character renders from all sides and switches texture expressions',async({page})=>{
  ...
  for(const [label,file] of [['斜め','quarter'],['正面','front'],['側面','side'],['背面','back']]) {
    ...
    await page.locator('#viewport canvas').screenshot({path:outputPath(`fes256-${file}.png`)});
  }
  ...
  for(const label of ['Happy','Angry','Surprised','Blink','Wink','Neutral']) {
    ...
    const frame=await page.locator('#viewport canvas').screenshot({path:outputPath(`fes256-${label.toLowerCase()}.png`)});
  ...
  await page.locator('#viewport canvas').screenshot({path:outputPath('fes256-rig.png')});
  ...
  await canvas.screenshot({path:outputPath('fes256-face.png')});
  await page.getByLabel('ワイヤーフレーム',{exact:true}).check();
  await canvas.screenshot({path:outputPath('fes256-face-wire.png')});
  await page.getByLabel('ワイヤーフレーム',{exact:true}).uncheck();
  await page.setViewportSize({width:390,height:844});
  await page.getByRole('button',{name:'斜め',exact:true}).click();
  await page.locator('#viewport canvas').screenshot({path:outputPath('fes256-mobile.png')});
