...
test('IRON YARD: real GLBs, mouse-look, WASD, boost, pause, resume and reset',async({page})=>{
  ...
  await page.screenshot({path:outputPath('game-deployment.png')});
  await page.getByRole('button',{name:/出撃する/}).click();
  await expectDragLookActive(page);
  await page.screenshot({path:outputPath('game-tps.png')});
  ...
  await page.screenshot({path:outputPath('game-boost.png')});
