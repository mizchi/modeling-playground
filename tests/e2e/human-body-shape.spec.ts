...
test('body sliders preserve parts, migrate settings, undo, persist and export shaped geometry',async({page})=>{
  ...
  for(const [name,values] of [['neutral',[0,0,0]],['slender',[-1,-1,0]],['full',[1,1,0]],['muscular',[0,-.5,1]]]){
    ...
    for(const view of ['front','side','back']){
      ...
      await page.locator('#viewport canvas').screenshot({path:outputPath(`human-body-${name}-${view}.png`)});
    }
  }
  ...
  await page.locator('#viewport canvas').screenshot({path:outputPath('human-body-male-muscular.png')});
  ...
  await page.locator('#viewport canvas').screenshot({path:outputPath('human-body-muscular-walk.png')});
