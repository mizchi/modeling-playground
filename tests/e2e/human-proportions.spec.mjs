...
test('height and leg sliders migrate, preserve edits, frame all views and export actual proportions',async({page})=>{
  ...
  for(const [name,height,leg] of [['short',-1,-1],['long-legs',0,1],['tall',1,1]]){
    ...
    for(const view of ['front','side','back','high']){
      await page.locator(`[data-view="${view}"]`).click();await canvas.screenshot({path:`output/human-proportions-${name}-${view}.png`});
    }
  }
  ...
  await canvas.screenshot({path:'output/human-proportions-short-face.png'});
  await set('height',1);await set('legLength',1);
  await canvas.screenshot({path:'output/human-proportions-tall-face.png'});
  ...
  await canvas.screenshot({path:'output/human-proportions-tall-walk.png'});
