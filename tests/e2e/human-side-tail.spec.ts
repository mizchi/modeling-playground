...
test('yellow side tail switches, persists, poses and exports on both bodies',async({page})=>{
  ...
  for(const view of ['front','quarter','side','back','high']){
    ...
    await canvas.screenshot({path:outputPath(`human-side-tail-${view}.png`)});
  }
  ...
  await canvas.screenshot({path:outputPath('human-side-tail-walk.png')});
  ...
  for(const time of [1,2,4,5]){
    ...
    await canvas.screenshot({path:outputPath(`human-side-tail-neck-${time}.png`)});
  }
