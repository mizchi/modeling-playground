...
import { createBase45 } from '../../human/models/base45/src/model.ts';
...
test('human workshop edits, switches modules, preserves history and exports a GLB',async({page})=>{
  ...
  for(const [view,name] of [['正面','front'],['側面','side'],['斜め下','low'],['斜め上','high'],['背面','back']]) {
    await page.getByRole('button',{name:view,exact:true}).click();await canvas.screenshot({path:outputPath(`human-${name}.png`)});
  }
  ...
  await page.getByRole('button',{name:'斜め',exact:true}).click();await page.screenshot({path:outputPath('human-workshop.png')});
...
test('compatible motion and skeleton imports work; invalid settings leave the character intact',async({page})=>{
  ...
  await page.locator('#face-file').setInputFiles(outputPath('lumi-face.png'));await expect(page.locator('#face-asset')).toHaveText('lumi-face.png');
  await page.getByRole('button',{name:'顔に寄る',exact:true}).click();
  await page.screenshot({path:outputPath('human-imported.png')});
...
test('human workshop fits a mobile viewport',async({page})=>{
  ...
  await page.screenshot({path:outputPath('human-mobile.png'),fullPage:true});
});
...
test('common neck is inspected in both models from profile, quarter and below',async({page})=>{
  ...
  for(const model of ['base45','lumi']) {
    ...
    for(const [view,name] of [['側面','side'],['斜め','quarter'],['斜め下','low']]) {
      ...
      await page.locator('#viewport canvas').screenshot({path:outputPath(`human-neck-${model}-${name}.png`)});
    }
  }
...
test('nape and neck motion survive exported GLB injection and are inspected from the rear quarter',async({page})=>{
  ...
  for(const model of ['base45','lumi']) {
    ...
    for(const [time,name] of [[0,'rest'],[1,'left'],[2,'right'],[4,'down'],[5,'up'],[8,'combined']]) {
      ...
      await canvas.screenshot({path:outputPath(`human-nape-${model}-${name}.png`)});
    }
  }
  await expect(page.locator('#error')).toBeHidden();
});
...
test('female body switches independently of hair and face, exports and plays shared motions',async({page})=>{
  ...
  for(const [view,name] of [['正面','front'],['側面','side'],['背面','back'],['斜め','quarter'],['斜め上','high'],['斜め下','low']]) {
    await page.getByRole('button',{name:view,exact:true}).click();await canvas.screenshot({path:outputPath(`human-female-${name}.png`)});
  }
  await page.locator('#wireframe').check();await canvas.screenshot({path:outputPath('human-female-wire.png')});await page.locator('#wireframe').uncheck();
  await page.locator('#hair').selectOption('lumi-short');await page.locator('#face').selectOption('lumi');
  ...
  await canvas.screenshot({path:outputPath('human-female-walk.png')});
  ...
  await page.locator('#motion').selectOption('rest');await page.screenshot({path:outputPath('human-female-workshop.png')});
  await expect(page.locator('#error')).toBeHidden();expect(errors).toEqual([]);
});
