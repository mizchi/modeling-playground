...
test('LUMI face and fitted hair are inspected from all sides, high and low',async({page})=>{
  ...
  for(const [yaw,pitch,name] of [[0,0,'face-front'],[40,0,'face-quarter'],[-40,0,'face-right'],[90,0,'face-side'],[145,20,'hair-back-high'],[180,0,'hair-back'],[40,25,'face-high'],[40,-30,'face-low']]) {
    ...
    let previous;
    await expect.poll(async()=>{const current=await canvas.screenshot(),stable=previous?.equals(current);previous=current;return stable;},{timeout:10_000,intervals:[200,300]}).toBe(true);
