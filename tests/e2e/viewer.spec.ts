...
const town = fileURLToPath(new URL('../../models/little-town/output/little-town.glb', import.meta.url));
...
test('Suzu renders through the shared viewer, including a face close-up and mobile',async ({page})=>{
  ...
  await expect(page.locator('#ik-panel')).toBeHidden();
  await page.getByRole('button',{name:'正面',exact:true}).click();
  await page.screenshot({path:outputPath('suzu-front.png')});
  ...
  await page.screenshot({path:outputPath('suzu-face.png')});
  await page.getByRole('button',{name:'全体を表示 F'}).click();
  await page.screenshot({path:outputPath('suzu-viewer.png')});
  ...
  await page.screenshot({path:outputPath('suzu-side.png')});
  await page.setViewportSize({width:390,height:844});
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
  await expect(page.locator('#model-name')).toHaveText('suzu.glb');
  expect(errors).toEqual([]);
...
test('Raven hovers, boosts once with replay, and slashes with its arm blade',async({page})=>{
  ...
  await page.screenshot({path:outputPath('raven-hover.png')});
  await page.getByRole('button',{name:'側面',exact:true}).click();
  await page.screenshot({path:outputPath('raven-side.png')});
  ...
  await page.screenshot({path:outputPath('raven-boost.png')});
  await page.getByRole('button',{name:'再生',exact:true}).click();
  ...
  for(const [t,name] of [['0.48','windup'],['0.7','slash'],['0.94','followthrough']]) {
    ...
    await page.screenshot({path:outputPath(`raven-${name}.png`)});
  }
  await page.getByLabel('再生位置').fill('0.78');
  for(const [view,name] of [['正面','front'],['側面','side'],['上面','top']]) {
    await page.getByRole('button',{name:view,exact:true}).click();
    await page.screenshot({path:outputPath(`raven-slash-${name}.png`)});
  }
  await page.getByRole('button',{name:'斜め',exact:true}).click();
  await page.getByRole('checkbox',{name:'骨格を表示'}).check();
  await page.screenshot({path:outputPath('raven-rig.png')});
  expect(errors).toEqual([]);
});
...
test('asset sidecars are validated, failures preserve the current model, and standalone GLB still works',async({page})=>{
  ...
  await expect(page.locator('#model-name')).toHaveText('raven.glb');
  await expect(page.getByRole('alert')).toBeHidden();
  await page.getByLabel('GLBファイルを選択').setInputFiles(fileURLToPath(new URL('../../robot/models/raven/output/raven.glb',import.meta.url)));
  await expect(page.locator('#status')).toHaveText('表示中');
  await page.getByLabel('アニメーションを選択').selectOption({label:'BladeSlash'});
  await page.getByLabel('再生位置').fill('0.78');
