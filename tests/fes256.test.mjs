...
import { FES256, EXPRESSION_NAMES } from '../models/fes256-definition.mjs';
import { createFes256Atlas, FES256_ATLAS } from '../models/fes256-texture.mjs';
...
test('six atlas tiles have unique pixels and export deterministically',async()=>{
  ...
  assert.deepEqual(await readFile(new URL('../output/fes256-expressions.png',import.meta.url)),encodeRgbaPng(a.image));
});

test('FES GLB embeds the atlas and expression contract deterministically without external images',async()=>{
  const bytes=await readFile(new URL('../output/fes256.glb',import.meta.url));
