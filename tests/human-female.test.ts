...
import { createHuman, disposeHuman, exportRig } from '../human/model.ts';
import { createMotions } from '../human/motion.ts';
...
test('delivered female GLB and quad source match the independent preset and validate',async()=>{
  ...
  const bytes=await readFile(new URL('../human/models/base45/output/human-female.glb',import.meta.url));
  ...
  const d=JSON.parse(await readFile(new URL('../human/models/base45/output/human-female.topology.json',import.meta.url),'utf8'));validateBaseTopology(d);
