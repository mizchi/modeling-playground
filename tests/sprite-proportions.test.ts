...
import {SPEC,PROPORTIONS,getRig,artifactNames,DIRECTIONS,PALETTE,sampleWalk,renderFrame,buildSheet,toRgba} from '../models/sprite-walk/src/walk.ts';
...
test('all proportion exports match their source, including retargeted pose metadata',()=>{
  for(const {id} of PROPORTIONS) {
    ...
    for(const [key,colored] of [['debug',true],['neutral',false]]) {
      assert.deepEqual(readFileSync(assetUrl(`${paths[key]}`)),
        encodeRgbaPng(toRgba(buildSheet({proportion:id,colored}))));
    }
    const metadata=JSON.parse(readFileSync(assetUrl(`${paths.metadata}`),'utf8'));
