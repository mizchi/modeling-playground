...
import { SPEC, DIRECTIONS, PALETTE, sampleWalk, renderFrame, buildSheet, toRgba } from '../models/sprite-walk/src/walk.ts';
...
test('delivered PNGs and pose metadata match the procedural source',()=>{
  for(const [name,colored] of [['debug',true],['neutral',false]]) {
    const bytes=readFileSync(assetUrl(`sprite-walk-${name}.png`));
    assert.deepEqual(bytes,encodeRgbaPng(toRgba(buildSheet({colored}))));
  }
  const metadata=JSON.parse(readFileSync(new URL('../models/sprite-walk/output/sprite-walk.json',import.meta.url),'utf8'));
