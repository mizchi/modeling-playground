...
import { STRIX_LEGS, STRIX_GAIT, STRIX_SPEC } from '../robot/models/strix/src/definition.ts';
import { strixPose } from '../robot/models/strix/src/motion.ts';
import { createStrix } from '../robot/models/strix/src/model.ts';
...
test('exported four-leg rig moves actual rigid armor and keeps feet level above ground',async()=>{
  ...
  const delivered=await readFile(new URL('../robot/models/strix/output/strix.glb',import.meta.url));
