...
import { frameModel, inspectModel, validateGlb, focusTarget } from '../viewer/model.ts';
...
test('the delivered town is accepted and truncated files are rejected', async () => {
  const buffer = await readFile(new URL('../models/little-town/output/little-town.glb', import.meta.url));
