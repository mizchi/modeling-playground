...
import { createWyvernRig } from '../models/wyvern/src/rig.ts';
import { createWyvern } from '../models/wyvern/src/model.ts';
import { WYVERN_FLIGHT, wyvernPose, wyvernClips } from '../models/wyvern/src/motion.ts';
...
test('delivered GLB retains its skeleton and wingbeat after reimport', async () => {
  const bytes = await readFile(new URL('../models/wyvern/output/wyvern.glb', import.meta.url));
