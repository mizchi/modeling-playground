...
import type { BastionSlot } from '../robot/models/bastion/src/definition.ts';
import { elementLookup, errorMessage } from './dom.ts';
import { BASTION_SLOTS, DEFAULT_LOADOUT, validateLoadout } from '../robot/models/bastion/src/definition.ts';
import { findBastion, replaceBastionPart } from '../robot/models/bastion/src/model.ts';

/** DOM adapter only: geometry and loadout state live in the model layer. */
export function createAssemblyPanel(onChange: ()=>void) {
  const $=elementLookup<PageElements & Record<`part-${BastionSlot}`,HTMLSelectElement>>(),fields=$('assembly-fields'),panel=$('assembly-panel');
  ...
  for(const slot of BASTION_SLOTS) {
    ...
    select.addEventListener('change',()=>{
      ...
      } catch(error){sync();$('assembly-status').textContent=errorMessage(error);}
    });
  ...
  $('assembly-export').addEventListener('click',async()=>{
    ...
    } catch(error){if(source===root)$('assembly-status').textContent=`保存できませんでした：${errorMessage(error)}`;}
