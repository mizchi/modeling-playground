...
export type ModelInfo = ReturnType<typeof inspectModel>;
import { Box3, Vector3 } from 'three';
export { modelMaterials, disposeModel } from '../modeling/resources.ts';

/** Return camera parameters without changing the model's original coordinates. */
export function frameModel(bounds: Box3, fov: number, aspect: number, direction = new Vector3(1, .85, 1.4)) {
...
export function inspectModel(model: Object3D) {
  ...
  model.traverse(object => {
    ...
    const instances = 'isInstancedMesh' in object ? (object as InstancedMesh).count : 1;
    meshes += instances;
...
export function focusTarget(object: Object3D, root: Object3D): Object3D {
  ...
  for (let ancestor: Object3D | null = object; ancestor && ancestor !== root; ancestor = ancestor.parent) {
    if (ancestor.userData.focusTarget && ancestor.parent !== root) return ancestor;
  }
  let content = root;
  while (content.children.length === 1 && !isMesh(content.children[0])) content = content.children[0];
